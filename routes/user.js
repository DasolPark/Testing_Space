var login = function(req, res){
	...
	//console.log(req); // 아무것도 출력되지 않음
	var database = req.app.get('database');//app.set('database', database)로 해놨기 때문에 가져올 수 있음
	//app은 왜 가져올 수 있나? 이 파일을 불러오는 initRoutes(app, router)함수에서 app을 가져왔기 때문에? 이유가 뭘까?
	//database.js에서 app.set('database', database);로 설정을 했음
	//그래서 express middleware에 database가 등록되었고, 따라서 req.app.get()으로 가져올 수 있음(expressjs.com참조)
	if(database.db){
